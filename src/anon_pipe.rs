...
use windows_sys::Win32::Foundation::{
    BOOL, DuplicateHandle, ERROR_ACCESS_DENIED, ERROR_BROKEN_PIPE, ERROR_HANDLE_EOF,
    ERROR_IO_PENDING, FALSE, GENERIC_READ, GENERIC_WRITE, GetLastError, INVALID_HANDLE_VALUE, TRUE,
    WAIT_OBJECT_0,
};
use windows_sys::Win32::Security::SECURITY_ATTRIBUTES;
use windows_sys::Win32::Storage::FileSystem::{
    CreateFileW, FILE_FLAG_FIRST_PIPE_INSTANCE, FILE_FLAG_OVERLAPPED, OPEN_EXISTING,
    PIPE_ACCESS_INBOUND, PIPE_ACCESS_OUTBOUND, ReadFile,
};
...
use windows_sys::Win32::System::Threading::{
    CreateEventW, GetCurrentProcess, GetCurrentProcessId, INFINITE, WaitForMultipleObjects,
};
...
pub fn read2(p1: AnonPipe, v1: &mut Vec<u8>, p2: AnonPipe, v2: &mut Vec<u8>) -> io::Result<()> {
    ...
    loop {
        let res = unsafe { WaitForMultipleObjects(2, objs.as_ptr(), FALSE, INFINITE) };
...
impl<'a> AsyncPipe<'a> {
    ...
    fn result(&mut self) -> io::Result<bool> {
        let amt = match self.state {
            ...
            State::Reading => overlapped_result(&self.pipe, &mut *self.overlapped, true)?,
            State::Read(amt) => amt,
        };
...
pub fn new_event(manual: bool, init: bool) -> io::Result<Handle> {
    unsafe {
        let event = CreateEventW(ptr::null_mut(), manual as BOOL, init as BOOL, ptr::null());
        if event.is_null() {
            Err(io::Error::last_os_error())
        } else {
